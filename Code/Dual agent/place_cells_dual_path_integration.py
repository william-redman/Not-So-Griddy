...
class PlaceCells(object):
    ...
    def get_nearest_cell_pos(self, activation, k=6):
        '''
        ...
        Returns:
            pred_pos: Predicted 2d position with shape [batch_size, sequence_length, 4].
        '''
        _, idxs = torch.topk(activation, k=k)
        pred_pos_indiv = self.us[idxs]
        pred_pos = torch.zeros([pred_pos_indiv.size()[0], pred_pos_indiv.size()[1], 2 * pred_pos_indiv.size()[3]])
        
        for ii in range(pred_pos.size()[0]):
            for jj in range(pred_pos.size()[1]):
                pred_pos_np, _ = kmeans(pred_pos_indiv.cpu()[ii, jj, :, :], 2)

                pred_pos[ii, jj, :] = torch.from_numpy(pred_pos_np.flatten())
        return pred_pos
