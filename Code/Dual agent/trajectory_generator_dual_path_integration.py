...
class TrajectoryGenerator(object):
    ...
    def get_generator(self, batch_size=None, box_width=None, box_height=None):
        ...
        while True:
            traj_self = self.generate_trajectory(box_width, box_height, batch_size)
            traj_other = self.generate_trajectory(box_width, box_height, batch_size)

            v = np.stack([traj_self['ego_v'] * np.cos(traj_self['target_hd']),
                          traj_self['ego_v'] * np.sin(traj_self['target_hd']),
                          traj_other['ego_v'] * np.cos(traj_other['target_hd']),
                          traj_other['ego_v'] * np.sin(traj_other['target_hd'])], axis=-1)
            v = torch.tensor(v, dtype=torch.float32).transpose(0, 1)

            pos = np.stack([traj_self['target_x'], traj_self['target_y'], traj_other['target_x'], traj_other['target_y']], axis=-1)
            pos = torch.tensor(pos, dtype=torch.float32).transpose(0, 1)
                        
            # Put on GPU if GPU is available
            pos = pos.to(self.options.device)
             
            place_outputs_self = self.place_cells.get_activation(pos[:, :, :2])
            #place_outputs_self[:, :, torch.arange(0, place_outputs_self.size()[2], 2)] = 0 # "true" social place cells
            place_outputs_other = self.place_cells.get_activation(pos[:, :, 2:])
            #place_outputs_other[:, :, torch.arange(1,  place_outputs_other.size()[2], 2)] = 0 # "true" social place cells
            place_outputs = place_outputs_self + place_outputs_other 
            place_outputs = torch.tensor(place_outputs, dtype=torch.float32)
            place_outputs = place_outputs.to(self.options.device)

            init_pos = np.stack([traj_self['init_x'], traj_self['init_y'], traj_other['init_x'], traj_other['init_y']], axis=-1)
            init_pos = torch.tensor(init_pos, dtype=torch.float32)
            init_pos = init_pos.to(self.options.device)
            
            init_actv_self = self.place_cells.get_activation(init_pos[:, :, :2]).squeeze()
            #init_actv_self[:, torch.arange(0, place_outputs_self.size()[2], 2)] = 0 # "true" social place cells
            init_actv_other = self.place_cells.get_activation(init_pos[:, :, 2:]).squeeze()
            #init_actv_other[:, torch.arange(1,  place_outputs_other.size()[2], 2)] = 0 # "true" social place cells
            init_actv = init_actv_self + init_actv_other
    ...
    def get_test_batch_single_agent(self, batch_size=None, box_width=None, box_height=None):
        ''' For testing performance, returns a batch of smample trajectories'''
        if not batch_size:
            batch_size = self.options.batch_size
        if not box_width:
        ...
        v = np.stack([traj_self['ego_v'] * np.cos(traj_self['target_hd']),
                      traj_self['ego_v'] * np.sin(traj_self['target_hd']),
                      np.zeros(np.shape(traj_self['ego_v'])),
                      np.zeros(np.shape(traj_self['ego_v']))], axis=-1)
        v = torch.tensor(v, dtype=torch.float32).transpose(0, 1)

        pos = np.stack([traj_self['target_x'], traj_self['target_y'], np.zeros(np.shape(traj_self['target_x'])), np.zeros(np.shape(traj_self['target_y']))], axis=-1)
        pos = torch.tensor(pos, dtype=torch.float32).transpose(0, 1)
        ...
        place_outputs_self = self.place_cells.get_activation(pos[:, :, :2])
        #place_outputs_self[:, :, torch.arange(0, place_outputs_self.size()[2], 2)] = 0 # "true" social place cells
        place_outputs = place_outputs_self
        place_outputs = torch.tensor(place_outputs, dtype=torch.float32)
        place_outputs = place_outputs.to(self.options.device)

        init_pos = np.stack([traj_self['init_x'], traj_self['init_y'], np.zeros(np.shape(traj_self['init_x'])), np.zeros(np.shape(traj_self['init_y']))], axis=-1)
        init_pos = torch.tensor(init_pos, dtype=torch.float32)
        init_pos = init_pos.to(self.options.device)
        
        init_actv_self = self.place_cells.get_activation(init_pos[:, :, :2]).squeeze()
        #init_actv_self[:, torch.arange(0, place_outputs_self.size()[2], 2)] = 0 # "true" social place cells
        init_actv = init_actv_self
