...
class GridScorer(object):
  ...
  def band_score(self, rm, res, box_width):
    """Get band score"""
    X = np.linspace(0., box_width, res)
    Y = np.linspace(0., box_width, res)
    k = np.arange(0., 2, 0.1)
    r2 = []
    l = [] 
    ...
    for ii in range(np.shape(k)[0]):
        for jj in range(np.shape(k)[0]):
            ...
            l.append(np.sqrt(k[ii]**2 + k[jj]**2))
    
    if np.sum(np.isnan(r2)) < len(r2):
        band_score = np.nanmax(r2)
        band_length = 1. / l[np.argwhere(r2 == np.nanmax(r2)).flatten()[0]] 
    else:
        band_score = np.nan
        band_length = np.nan
    
    return band_score, band_length


  def get_sac_interp(self, cell):
    """Get interpolated sac."""
    sac = self.calculate_sac(cell)
    xx = np.linspace(-1, 1, 39)
    yy = np.linspace(-1, 1, 39)
    return scipy.interpolate.RegularGridInterpolator((xx, yy), sac)

  def get_phi(self, cell, interp=None, spacing_values=np.arange(0.01, 1.0, 0.01)): #0.15
